...
function test() {
    ...
    $backend = new PHPSerializedODB_WithWritepack('test-writepack-debug');
    $repo = git_repository_new();
    $odb = git_odb_new();
    ...
    //$wp = $backend->writepack($odb,'Git2Test\ODBWritepack\do_fail','DEBUG');
    $wp = git_odb_write_pack($odb,'Git2Test\ODBWritepack\do_fail','DEBUG');
    $t = $wp->backend->version;
    //$wp->append(hex2bin($pack),$stats);
    $wp->append(hex2bin($pack));
    $wp->commit();
}
...
//testbed_test('DEBUG-1','Git2Test\ODBWritepack\test');
